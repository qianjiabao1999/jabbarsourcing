...
const quickSource = await readFile(new URL("../assets/container-visual.js", import.meta.url), "utf8");
...
assert.match(source, /Analyzer\.prototype\.renderContainer[\s\S]*var loads = this\.visibleContainerLoads\(estimate, MAX_CONTAINER_BARS\)/, "page cards must use the shared per-container loads");
assert.equal(count(/drawContainerLoadBars\(/g), 1, "the single 4K PNG overview path must use per-container bars");
assert.equal(count(/setAttribute\("data-container-load"/g), 1, "analyzer fallback allocation hook count");

for (const token of [
  "CONTAINER_CAPACITY_CBM", "CONTAINER_EPSILON_CBM", "entries", "data-container-count",
  "data-container-load", "data-container-index", "cbm-container-visual", "cbm-container-fill", "is-full"
]) {
  assert(quickSource.includes(token), `missing quick-calculator full-first allocation token ${token}`);
}
assert.match(quickSource, /Math\.ceil\(\(totalCbm - CONTAINER_EPSILON_CBM\) \/ CONTAINER_CAPACITY_CBM\)/, "quick calculator must tolerate floating-point noise at exact-capacity boundaries");
assert.match(quickSource, /totalCbm - index \* CONTAINER_CAPACITY_CBM/, "quick calculator must allocate each container in order");
assert.doesNotMatch(quickSource, /totalCbm\s*\/\s*\(count\s*\*\s*CONTAINER_CAPACITY_CBM\)/, "quick calculator must not average loads across containers");
